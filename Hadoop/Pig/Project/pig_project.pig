-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/Amruta/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/Amruta/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/Amruta/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/Amruta/outputs;

-- Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/Amruta/outputs' USING PigStorage('\t');
